...
```
# Create a "connection"
s = ServerAccessOptions(url, authentication, repository_id, base_prefix, ...)
...
side effect: adds a list of triples to rdf_obj

#### add_triple(rdf_obj, triple)
side effect: adds a triple to rdf_obj
...
#### AnonymousTriple(predicate, object)
...
#### Identifier(label, sao, generate_on_fail)
basically a lookup function for that label, if lookup fails, generates a UUID with the base prefix
...
## Mechanism to read and parse files and a logging system

#### xml2rdf(filename, upper_level_extractor)

upper_level_extractor is a function that takes in a XML node and returns RDF

#### log_event(filename, datetime, event)
Logs an event during the processing of filename at datetime.
should probably use a poratable SQL database for this.

### log_statistics
displays statistics from the log
...
What to consider before publishing the code? Is this design sensible for one library (two libraries)?

### Performance

- code is slow: how best to profile?


### Robustness

- graceful degradation: often the XML is mangled and errors are generated and execution stops: how to handle? use tryCatch?
- correctness: how to analyze the correctness of the code? use the logging system?

### Complexity

- code is too complex, hard time to find the correct function: is S4 a good solution?
